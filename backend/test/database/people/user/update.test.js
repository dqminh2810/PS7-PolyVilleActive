...
describe('Update visitor document', () => {
    ...
    it('Update one user using instance', () => {
        //useful to update multiple fields of the object
        return assertHelper(user.updateOne({ name: "update test name user" }));
    });

    it('Update multiple users matching using model', () => {
        return assertHelper(User.updateMany({ name: "test name user" }, { name: "update test name user" }));
    });

    it('Update one user using model', () => {
        return assertHelper(User.findOneAndUpdate({ name: "test name user" }, { name: "update test name user" }));
    });

    it('Update one user with id using model', () => {
        return assertHelper(User.findByIdAndUpdate(user._id, { name: "update test name user" }));
    });

    function assertHelper(statement) {
        statement.then(() => User.find({})
            .then((users) => {
                assert(users.length === 1);
                assert(String(users[0]._id) === String(user._id));
                assert(String(users[0].details_user._id) === String(organizer._id));
                assert(String(users[0].participate_to[0]._id) === String(event1._id));
                assert(String(users[0].participate_to[1]._id) === String(event2._id));
                assert(String(users[0].details_user.organize[0]) === String(event1._id));
                assert(String(users[0].details_user.organize[1]) === String(event2._id));
                assert(users[0].name === "update test name user");
