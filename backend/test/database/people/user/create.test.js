...
describe('Create user document', () => {
    ...
    it('Create new user ', () => {
        ...
        return user.save()
            .then((doc) => {
                assert(!user.isNew);
                assert(String(doc.details_user._id) === String(visitor._id));
                assert(String(doc.participate_to[0]._id) === String(event1._id));
                assert(String(doc.participate_to[1]._id) === String(event2._id));
                assert(String(doc.details_user.preferences[0]) === String(event1._id));
                assert(String(doc.details_user.preferences[1]) === String(event2._id));
