...
describe('Delete visitor document', () => {
    ...
    it('Delete one visitor using its instance', () => {
        return visitor.remove()
            .then(() => Visitor.findOne({ _id: visitor._id }))
            .then((visitor) => {
                assert(visitor == null);
            })

    });
    it('Delete multiple visitors matching using model', () => {
        return Visitor.deleteMany({ _id: visitor._id })
            .then(() => Visitor.findOne({ _id: visitor._id })
                .then((event) => {
                    assert(event == null);
                })
    ...
    it('Delete one visitor matching using model' , () => {
        return Visitor.findOneAndDelete({ _id: visitor._id })
            .then(() => Visitor.findOne({ _id: visitor._id })
                .then((visitor) => {
                    assert(visitor == null);
                })
    ...
    it('Delete one visitor using id', () => {
        return Visitor.findByIdAndDelete(visitor._id)
