...
describe('Create organizer document', () => {
    let country, city, cinema, restaurant, library, sportCenter, event1, event2;
    beforeEach(() => {
        country = new Country({
    ...
    it('Create new visitor ', () => {
        const visitor = new Visitor({
            preferences: [event1._id, event2._id]
        });
        return visitor.save()
            .then((doc) => {
                assert(!visitor.isNew);
                assert(String(doc.preferences[0]) === String(event1._id));
                assert(String(doc.preferences[1]) === String(event2._id));
            })
    });
});
